    ...
    loadUserHooks: function (cb) {
      ...
      async.auto({
        ...
        nodeModulesFolder: function(cb) {
          includeAll.optional({
            dirname: path.resolve(sails.config.monorepoNodeModulesContainerPath || sails.config.appPath, 'node_modules'),
            filter: /^(package\.json)$/,
            excludeDirs: /^\./,
            ...
            modules = (function _flatten(modules, installedHooks, currentPath, level) {
              ...
              Object.keys(modules).forEach(function(identity) {
                ...
                if (identity === 'package.json' && modules[identity] === true) {
                  var filePath = path.resolve(sails.config.monorepoNodeModulesContainerPath || sails.config.appPath, 'node_modules', currentPath, identity);
                  try {
        ...
        try {
          ...
          _.extend(hooks, _.reduce(results.nodeModulesFolder, function(memo, modulePackageJson, identity) {
            ...
            // Load the hook code
            var hook = require(path.resolve(sails.config.monorepoNodeModulesContainerPath || sails.config.appPath, 'node_modules', identity));
